...
export default function ScenarioDemoPanel({
  ...
  const completedCount = [...completedIds].filter((id) => primaryScenarios.some((s) => s.id === id)).length
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Primary Demo: Baselines + 2025 Incident Recreations */}
      <div className="space-y-5">
        {primaryScenarios.map((scenario, idx) => {
          ...
          return (
            <div key={scenario.id}>
              ...
              {/* Phase 2 header — before first incident replay */}
              {scenario.id === 101 && (
                        ...
                        {verdict.anomalyScore != null && (
                          <div className="mt-3 pt-3 border-t border-gray-700/50">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <span className={`text-2xl font-black ${
                                verdict.anomalyScore >= threshold ? 'text-red-400' : verdict.anomalyScore >= 25 ? 'text-orange-400' : 'text-green-400'
                              }`}>
                  ...
                  <div className="flex flex-col items-end gap-3 pt-2 shrink-0">
                    {isActive && runState === 'running' ? (
                      ...
                      <button
                        onClick={() => runScenario(scenario)}
                        disabled={runState === 'running' || runningAll}
                        className="px-7 py-3.5 text-lg font-bold rounded-2xl transition-colors bg-red-600 hover:bg-red-700 disabled:bg-gray-700 text-white"
                      >
                        Launch Attack
                      </button>
                    )}
                          ...
                          >
                            <div className="flex-shrink-0">
                              {isCurrent ? (
                                ...
                                <svg className={`w-6 h-6 ${
                                  i >= scenario.steps.length - 2 ? 'text-red-400' : 'text-gray-500'
                                }`} fill="currentColor" viewBox="0 0 20 20">
                                  <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                                </svg>
                              ) : (
