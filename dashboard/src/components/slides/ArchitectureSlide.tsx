...
const steps = [
  ...
  { num: '6', label: 'AI Model 2', desc: 'Secondary model', color: 'text-purple-400', bg: 'bg-purple-400/10', border: 'border-purple-400/30' },
...
export default function ArchitectureSlide() {
  return (
    <SlideLayout>
      ...
      <p className="text-xl text-gray-400 mb-8">8-Step Verdict Pipeline</p>

      <div className="flex items-center justify-between gap-1 mb-8">
        {steps.map((step, i) => (
          <div key={step.num} className="flex items-center gap-1">
            <div className={`${step.bg} border ${step.border} rounded-xl p-3 text-center min-w-[100px]`}>
              <div className={`text-xs ${step.color} mb-0.5`}>Step {step.num}</div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-5">
          ...
          <p className="text-base text-gray-300">
            Action forwarded to target contract. Stats updated, rate limit window tracked, daily volume recorded. Full audit trail on-chain.
          </p>
        </div>
