...
const BEHAVIORAL_DIMENSIONS = [
  { name: 'Value Deviation', max: 25, total: 155, desc: 'Flags transactions significantly above the agent\'s historical average', example: 'Agent usually sends 0.1 ETH, suddenly tries 50 ETH', color: 'bg-red-500' },
  { name: 'Contract Diversity', max: 20, total: 155, desc: 'Detects when agent suddenly interacts with unknown contracts', example: 'DeFi agent starts calling NFT minting contracts', color: 'bg-orange-500' },
  { name: 'Velocity', max: 15, total: 155, desc: 'Catches burst-rate transactions faster than the agent\'s baseline', example: '20 transactions in 60 seconds vs. normal 2/minute', color: 'bg-yellow-500' },
  { name: 'Function Pattern', max: 30, total: 155, desc: 'Identifies unusual function calls for this agent type', example: 'Swap agent calling approve() on an unknown token', color: 'bg-purple-500' },
  { name: 'Time-of-Day', max: 10, total: 155, desc: 'Flags activity outside the agent\'s normal operating hours', example: 'Business-hours agent active at 3 AM UTC', color: 'bg-blue-500' },
  { name: 'Sequential Probing', max: 35, total: 155, desc: 'Detects monotonically increasing values (threshold hunting)', example: 'Agent sends 1, 2, 4, 8, 16 ETH probing for limits', color: 'bg-pink-500' },
  { name: 'Cumulative Drift', max: 20, total: 155, desc: 'Catches slow baseline poisoning over many transactions', example: 'Gradually increasing average over 100 small transactions', color: 'bg-teal-500' },
]
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          {EXPLOITS.map((e) => (
            ...
            >
              ...
              <div className="text-red-400 text-3xl font-black mb-2">{e.amount}</div>
              <p className="text-gray-400 text-base">{e.method}</p>
            </div>
          ))}
        </div>

        <div className="bg-red-500/5 border border-red-500/20 rounded-xl p-5 text-center">
          <div className="text-red-400 text-4xl font-black mb-1">$2B+ lost</div>
      ...
      <Section id="defense" borderColor="border-l-orange-500">
        ...
        <div className="relative flex items-center justify-center mb-8 py-4">
          <div className="relative w-full max-w-xl aspect-square rounded-full border-2 border-dashed border-purple-500/40 flex items-center justify-center p-6 sm:p-10">
            <span className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gray-900 px-4 text-purple-400 text-base font-black uppercase tracking-wider">
              ...
              <div key={layer.layer} className={`${layer.bg} rounded-xl border border-gray-800 overflow-hidden`}>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-white font-bold text-xl">{layer.name}</div>
                ...
                >
                  <div className="px-5 pb-5 border-t border-gray-800/50 pt-4">
                    <p className={`${layer.textColor} font-bold mb-3 italic text-xl`}>
                      {layer.tagline}
                    </p>
                    <ul className="space-y-1.5">
                      {layer.details.map((d, i) => (
                        <li key={i} className="text-gray-400 text-lg flex items-start gap-2">
      ...
      <Section id="pipeline" borderColor="border-l-cyan-500">
        ...
        <div className="mt-4 bg-red-500/5 border border-red-500/20 rounded-xl p-5 flex items-start gap-3">
          ...
          <div>
            <div className="text-red-400 font-bold text-base mb-1">When a proposal is DENIED</div>
                ...
                >
                  ...
                  <div className="ml-8">
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-sm">
                      <span className="text-gray-500 font-mono bg-gray-800 px-2.5 py-1 rounded">AccessControl</span>
                      <span className="text-gray-500 font-mono bg-gray-800 px-2.5 py-1 rounded">Pausable</span>
                    </div>
                  </div>
                </button>
                <div
                ...
                >
                  <div className="px-5 pb-5 border-t border-gray-700/50 pt-4 space-y-4">
                    <div className="flex items-center gap-2">
                      ...
                      <code className="text-sm font-mono text-cyan-400 bg-gray-800 px-2.5 py-1 rounded">
                        0x5F938e4c62991Eb4af3Dd89097978A1f376e6CC8
                      </code>
                    </div>
                    <div>
                      <div className="text-gray-500 text-sm uppercase tracking-wider mb-2">Key Functions</div>
                      <div className="flex flex-wrap gap-2">
                        {['processVerdict(bytes)', 'unfreezeAgent(bytes32)', 'getAgentPolicy(bytes32)', 'checkAll(CheckParams)'].map((fn) => (
                          <code key={fn} className="text-sm font-mono text-green-400/80 bg-green-400/5 px-2.5 py-1 rounded border border-green-400/10">
                            {fn}
                          </code>
                        ))}
                      </div>
                    ...
                    <div>
                      <div className="text-gray-500 text-sm uppercase tracking-wider mb-2">Core Logic</div>
                      <pre className="bg-gray-950 rounded-lg p-4 overflow-x-auto border border-gray-800 text-sm">
              ...
              <div className="bg-gray-800/50 rounded-xl border border-gray-700/50 overflow-hidden">
                ...
                >
                  ...
                  <div className="ml-8">
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-sm">
                      <span className="text-gray-500 font-mono bg-gray-800 px-2.5 py-1 rounded">Ownable</span>
                    </div>
                  </div>
                </button>
                <div
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {['registerAgent(bytes32, string, string)', 'getAgent(bytes32)', 'getAgentCount()'].map((fn) => (
                          <code key={fn} className="text-sm font-mono text-blue-400/80 bg-blue-400/5 px-2.5 py-1 rounded border border-blue-400/10">
                            {fn}
                          </code>
                        ))}
                      </div>
          ...
          {BEHAVIORAL_DIMENSIONS.map((dim) => {
            ...
            return (
              <div key={dim.name} className="bg-gray-800/50 rounded-xl border border-gray-700/50 p-4">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <span className="text-white font-bold text-lg">{dim.name}</span>
                  </div>
                ...
                <p className="text-gray-400 text-base">{dim.desc}</p>
                ...
                >
                  <div className="bg-gray-900/80 rounded-lg px-3 py-2 border border-gray-800">
                    <span className="text-gray-600 text-sm uppercase tracking-wider">Example: </span>
                    <span className="text-gray-400 text-sm italic">{dim.example}</span>
                  </div>
                </div>
              </div>
            )
      ...
      <Section id="techstack" borderColor="border-l-gray-500">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {TECH_BADGES.map((badge) => (
            <span
              key={badge}
              className="text-base font-mono text-gray-300 bg-gray-800 px-4 py-2 rounded-lg border border-gray-700"
            >
              {badge}
            </span>
          ))}
