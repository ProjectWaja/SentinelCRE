...
function deterministicEvaluate(proposal: any, overrides?: PolicyOverrides): {
  ...
  if (checkValue && value > effectiveMaxValue) {
    ...
    return {
      v1: { verdict: 'DENIED', confidence: 95, reason },
  ...
  if (checkMint && mint > effectiveMaxMint) {
    ...
    layerInfo.caughtBy = 'layer1'
    return {
      v1: { verdict: 'DENIED', confidence: 98, reason },
      v2: { verdict: 'DENIED', confidence: 97, reason },
      anomalyScore: 0, anomalyFlagged: false, anomalyDimensions: [],
      layerCatchInfo: layerInfo,
    }
  }
  ...
  if (checkTarget && !APPROVED_CONTRACTS.includes(target)) {
    ...
    layerInfo.caughtBy = 'layer1'
    return {
      v1: { verdict: 'DENIED', confidence: 92, reason },
      v2: { verdict: 'DENIED', confidence: 90, reason },
      anomalyScore: 0, anomalyFlagged: false, anomalyDimensions: [],
      layerCatchInfo: layerInfo,
    }
  }
  ...
  if (checkFunction && BLOCKED_FUNCTIONS.includes(fn)) {
    ...
    layerInfo.caughtBy = 'layer1'
    return {
      v1: { verdict: 'DENIED', confidence: 96, reason },
      v2: { verdict: 'DENIED', confidence: 95, reason },
      anomalyScore: 0, anomalyFlagged: false, anomalyDimensions: [],
      layerCatchInfo: layerInfo,
    }
  }
  ...
  if (checkRate) {
    ...
    if (actionCount > effectiveRateLimit) {
      ...
      layerInfo.caughtBy = 'layer1'
      return {
        v1: { verdict: 'DENIED', confidence: 90, reason },
        v2: { verdict: 'DENIED', confidence: 88, reason },
        anomalyScore: 0, anomalyFlagged: false, anomalyDimensions: [],
        layerCatchInfo: layerInfo,
      }
    }
  ...
  if (checkPoR && mint > 0n) {
    ...
    if (requiredReserves > simulatedReserves) {
      ...
      return {
        ...
        v2: { verdict: 'DENIED', confidence: 92, reason },
        anomalyScore: 0, anomalyFlagged: false, anomalyDimensions: [],
        layerCatchInfo: layerInfo,
      }
    }
  ...
  if (recentValues.length >= 2) {
    ...
    if (isIncreasing) {
      dimensions.push({ name: 'Sequential Probing', fired: true, score: 35, reason: `${recentValues.length + 1} monotonically increasing values detected` })
      anomalyScore += 35
    }
  }
  ...
  if (recentValues.length > 0) {
    ...
    if (currentEth > avg * 2 && avg > 0) {
      dimensions.push({ name: 'Value Deviation', fired: true, score: 12, reason: `${currentEth.toFixed(1)} ETH is ${(currentEth / avg).toFixed(1)}x above moving average` })
      anomalyScore += 12
    }
  }
  ...
  if (INJECTION_PATTERNS.some(p => desc.includes(p))) {
    dimensions.push({ name: 'Prompt Injection', fired: true, score: 40, reason: 'Injection pattern detected in action description' })
    anomalyScore += 40
  }
  ...
  if (desc.includes('transfer all')) {
    dimensions.push({ name: 'Value Deviation', fired: true, score: 25, reason: 'Fund extraction language detected — "transfer all" pattern' })
    anomalyScore += 25
  }
  ...
  if (recentValues.length >= 5) {
    ...
    if (origin > 0 && currentEth > origin * 3) {
      dimensions.push({ name: 'Cumulative Drift', fired: true, score: 20, reason: `Current ${currentEth.toFixed(1)} ETH is ${((currentEth / origin - 1) * 100).toFixed(0)}% above origin baseline` })
      anomalyScore += 20
    }
  }
  ...
  if (anomalyFlagged) {
    ...
    return {
      v1: { verdict: 'DENIED', confidence: 88, reason },
      v2: { verdict: 'DENIED', confidence: 85, reason },
  ...
  return {
    v1: { verdict: 'APPROVED', confidence: 92, reason: 'Action within policy limits — value, target, function all compliant' },
    v2: { verdict: 'APPROVED', confidence: 90, reason: 'No anomalies detected — behavioral profile normal' },
    anomalyScore, anomalyFlagged, anomalyDimensions: dimensions,
    layerCatchInfo: layerInfo,
  }
}
