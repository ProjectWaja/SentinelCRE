 ...
 *
 * Dimensions:
 *   1. Value Deviation    (+25) — value > 2.5σ from historical mean
 *   2. Contract Diversity (+20) — first interaction with unknown contract
 *   3. Velocity Scoring   (+15) — action interval < 50% of expected
 *   4. Function Pattern   (+30) — unusual function signature for this agent
 *   5. Time-of-Day        (+10) — activity outside normal hours
 *   6. Sequential Probing (+35) — 3+ monotonically increasing values
 *   7. Cumulative Drift   (+20) — rolling avg drifted from frozen origin baseline
 *
 * Total possible: 155. Default threshold: 50.
 */
...
// ── Dimension 1: Value Deviation (+25) ───────────────────────────────
...
// ── Dimension 2: Contract Diversity (+20) ────────────────────────────
...
export function checkContractDiversity(
  ...
  if (knownContracts.length === 0) {
    return { name, score: 10, maxWeight, fired: true, reason: 'New agent — no contract history yet' }
  }
...
// ── Dimension 3: Velocity Scoring (+15) ──────────────────────────────
...
export function checkVelocity(
  ...
  if (interval < minExpectedInterval * 0.5) {
    return {
      name,
      score: 15,
      maxWeight,
  ...
  if (interval < minExpectedInterval * 0.75) {
    return {
      name,
      score: 8,
      maxWeight,
...
// ── Dimension 4: Function Pattern (+30) ──────────────────────────────
...
// ── Dimension 6: Sequential Probing (+35) — the hero feature ─────────
...
export function checkSequentialProbing(
  ...
  if (monotonicallyIncreasing && window.length >= 3) {
    ...
    return { name, score: 35, maxWeight, fired: true, reason }
  }

  return { name, score: 0, maxWeight, fired: false, reason: 'No sequential escalation pattern' }
}

// ── Dimension 7: Cumulative Drift (+20) — the "boiling frog" defense ──
